...
void main() {
	...
	if ((nGlobal < 6)) {
		SetCommandable(1, oBendakstar021);
		SignalEvent(oBendakstar021, EventUserDefined(5000));
	}
	AssignCommand(GetFirstPC(), ClearAllActions());
	AssignCommand(GetFirstPC(), ActionJumpToObject(GetObjectByTag("tar02_spbendak", 0), 0));
	int3 = 0;
	while ((int3 < GetGlobalNumber("TAR_ARENANPCS"))) {
		int4 = GetGlobalNumber(("TAR_ARENANPC" + IntToString(int3)));
		switch (int4) {
			case 0:
				string1 = "bastila";
				break;
		...
		AddPartyMember(int4, GetObjectByTag(string1, 0));
		(int3++);
	}
}
