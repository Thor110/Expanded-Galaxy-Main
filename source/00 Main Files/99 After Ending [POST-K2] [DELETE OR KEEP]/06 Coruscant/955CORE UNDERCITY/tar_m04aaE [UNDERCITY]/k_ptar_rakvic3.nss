...
void main() {
	...
	int int1;
	object oNearestTar04_rakghoulvic;
	int1 = 0;
	while ((int1 < 3)) {
		oNearestTar04_rakghoulvic = GetNearestObjectByTag("tar04_rakghoulvic", OBJECT_SELF, (int1 + 1));
		ChangeToStandardFaction(oNearestTar04_rakghoulvic, 1);
		DelayCommand(1.0, AssignCommand(oNearestTar04_rakghoulvic, sub1(OBJECT_INVALID)));
		(int1++);
	}
	DestroyObject(oRakghoulvic00, 0.0, 1, 0.0);
	DestroyObject(oRakghoulvic01, 0.0, 1, 0.0);
	DestroyObject(oRakghoulvic02, 0.0, 1, 0.0);
}
