...
void main() {
	...
	if (((oEntering == GetPartyMemberByIndex(int1)) && (!sub1(OBJECT_SELF)))) {
		sub2("CLOSING GATE", 5, 5, 5.0);
		sub3(OBJECT_SELF, 1);
		sub4(oTar04_maingate, intGLOB_1, 0);
		AssignCommand(oTar04_maingate, ActionCloseDoor(oTar04_maingate));
		DelayCommand(1.0, SignalEvent(oHendarsfate, EventUserDefined(3000)));
		AssignCommand(oOutcastWoman043, ActionResumeConversation());
	}
}
