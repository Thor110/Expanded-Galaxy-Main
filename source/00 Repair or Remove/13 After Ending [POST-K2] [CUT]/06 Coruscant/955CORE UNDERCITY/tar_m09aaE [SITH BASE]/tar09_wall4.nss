void main() {
	...
	if ((GetLocked(GetObjectByTag("wall5", 0)) == 0)) {
		AssignCommand(GetObjectByTag("wall5", 0), PlayAnimation(201, 1.0, 0.0));
		SetLocked(GetObjectByTag("wall5", 0), 1);
	}
	else {
		AssignCommand(GetObjectByTag("wall5", 0), PlayAnimation(200, 3.0, 0.0));
		SetLocked(GetObjectByTag("wall5", 0), 0);
	}
