			...
			if ((nRandom == 3)) {
				...
				if ((((nRandom == 4) || (nRandom == 5)) && (GetRacialType(OBJECT_SELF) != 5))) {
					if ((GetGender(OBJECT_SELF) == 0)) {
						...
						ActionDoCommand(sub6(intGLOB_46, 1));
					}
					else {
...
void main() {
	...
	if ((int1 == 1001)) {
		object oNearestTar02_wpsweep = GetNearestObjectByTag("tar02_wpsweep", OBJECT_SELF, 1);
		if (sub1(OBJECT_SELF, intGLOB_119)) {
			...
			if (sub1(OBJECT_SELF, intGLOB_118)) {
				...
				if ((GetDistanceBetween(OBJECT_SELF, oNearestTar02_wpsweep) < 4.0)) {
					sub8("SWEEPING", 5, 5, 5.0);
					ClearAllActions();
					ActionMoveToObject(oNearestTar02_wpsweep, 0, 1.0);
					ActionPlayAnimation(118, 1.0, 0.0);
					ActionPlayAnimation(118, 1.0, 0.0);
					ActionPlayAnimation(118, 1.0, 0.0);
					ActionDoCommand(sub2(OBJECT_SELF, intGLOB_119, 1));
				}
			}
		}
	}
