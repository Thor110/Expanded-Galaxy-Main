...
void main() {
	...
	object oSithGuard = GetObjectByTag("SithGuard", 0);
	int int1 = ((GetIsObjectValid(GetItemPossessedBy(GetFirstPC(), "ptar_sithpapers")) && sub1(oSithGuard)) || sub2(OBJECT_INVALID));
	if (((!sub1(OBJECT_SELF)) && GetIsPC(oEntering))) {
		sub3(OBJECT_SELF, 1);
		sub4("SithGuard", "", GetObjectByTag("tar02_wppcguard", 0));
	}
}
