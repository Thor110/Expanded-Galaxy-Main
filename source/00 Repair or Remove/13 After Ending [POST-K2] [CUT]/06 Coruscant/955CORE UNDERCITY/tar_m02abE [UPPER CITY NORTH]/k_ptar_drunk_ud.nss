...
void sub3() {
	if (((!GetIsConversationActive()) && (GetStandardFaction(OBJECT_SELF) != 1))) {
		switch (Random(2)) {
			case 0:
				ActionPlayAnimation(13, 1.0, 3.0);
				break;
			default:
				string string1 = GetTag(OBJECT_SELF);
				if ((string1 != "Drunk021")) {
					SetFacingPoint(GetPosition(GetObjectByTag("drunk021", 0)));
				}
				else {
					SetFacingPoint(GetPosition(GetObjectByTag("drunk022", 0)));
				}
								...
								if ((int1 == 1008)) {
									...
									if ((int1 == 1009)) {
										...
										if ((int1 == 1010)) {
											...
											if ((int1 == 2000)) {
												ClearAllActions();
												sub4(GetTag(OBJECT_SELF), "tar02_drunk021", OBJECT_INVALID);
											}
											else {
												if ((int1 == 3000)) {
