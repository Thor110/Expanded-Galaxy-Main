...
void main() {
	...
	object oTar08_tortcage = GetObjectByTag("tar08_tortcage", 0);
	object oHudrow081 = GetObjectByTag("hudrow081", 0);
	object oTotureCage = GetObjectByTag("TotureCage", 0);
	if ((((GetDistanceBetween(oNearest, OBJECT_SELF) > 15.0) && (GetGlobalBoolean("Tar_HudrowFree") == 0)) || (!GetIsObjectValid(oNearest)))) {
		SetGlobalBoolean("Tar_HudrowFree", 1);
		if (GetIsObjectValid(oTotureCage)) {
			sub1("CAGE SND OFF", 5, 5, 5.0);
			SoundObjectStop(oTotureCage);
		}
		else {
			sub1("BAD CAGE SND", 5, 5, 5.0);
		}
		AssignCommand(oTar08_tortcage, sub2());
		AssignCommand(oHudrow081, ClearAllEffects());
		sub3("Hudrow081", "tar08_hudrow081", OBJECT_INVALID);
	}
}
