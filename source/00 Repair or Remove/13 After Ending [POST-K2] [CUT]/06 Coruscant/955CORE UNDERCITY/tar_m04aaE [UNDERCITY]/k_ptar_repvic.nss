...
void sub2(object objectParam1) {
	...
	DelayCommand(0.7, ApplyEffectAtLocation(0, EffectVisualEffect(1047, 0), location1, 0.0));
	DelayCommand(0.8, sub3(objectParam1, GetLocation(oPtar_wpgraveyard)));
	DelayCommand(1.1, PlaySound("cs_ghoulchange"));
	DelayCommand(1.2, ApplyEffectAtLocation(0, EffectVisualEffect(1046, 0), location1, 0.0));
	DelayCommand(1.4, sub4(location1));
}
...
void main() {
	...
	sub1(7.0);
	ApplyEffectToObject(1, EffectHorrified(), oRepublicso, 4.0);
	DelayCommand(2.0, sub2(oRepublicso));
}
