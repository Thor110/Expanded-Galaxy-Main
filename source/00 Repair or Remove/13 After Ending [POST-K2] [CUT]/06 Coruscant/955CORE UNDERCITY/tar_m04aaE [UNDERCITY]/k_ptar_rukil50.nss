...
void main() {
	sub1(GetFirstPC());
	SetGlobalNumber("Tar_Rukil", 50);
	DestroyObject(GetItemPossessedBy(GetFirstPC(), "ptar_appjournal"), 0.0, 0, 0.0);
	DestroyObject(GetItemPossessedBy(GetFirstPC(), "ptar_rukjournal"), 0.0, 0, 0.0);
	DestroyObject(GetItemPossessedBy(GetFirstPC(), "ptar_rukjournal"), 0.0, 0, 0.0);
	object oRukil04 = GetObjectByTag("rukil04", 0);
	object oTar04_wprukilbadstart = GetObjectByTag("tar04_wprukilbadstart", 0);
	AssignCommand(oRukil04, ClearAllActions());
	AssignCommand(oRukil04, ActionJumpToObject(oTar04_wprukilbadstart, 0));
