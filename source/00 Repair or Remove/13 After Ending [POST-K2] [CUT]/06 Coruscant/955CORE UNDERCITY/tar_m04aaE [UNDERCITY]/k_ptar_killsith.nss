...
void main() {
	...
	if ((GetIsPC(oEntering) && (!sub1(OBJECT_SELF)))) {
		sub2(OBJECT_SELF, 1);
		DestroyObject(GetObjectByTag("tar04_sithcomm0", 0), 0.0, 1, 0.0);
		DestroyObject(GetObjectByTag("tar04_sithtrooper0", 1), 0.0, 1, 0.0);
		DestroyObject(GetObjectByTag("tar04_sithtrooper0", 0), 0.0, 1, 0.0);
	}
}
