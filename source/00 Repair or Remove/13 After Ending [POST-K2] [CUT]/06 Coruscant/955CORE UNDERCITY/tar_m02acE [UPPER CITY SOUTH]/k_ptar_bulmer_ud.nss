	...
	switch (int1) {
		...
		case 2000:
			if ((!sub1(OBJECT_SELF, intGLOB_118))) {
				...
				if ((GetDistanceBetween(OBJECT_SELF, GetFirstPC()) < 20.0)) {
					object oNPC;
					int int6 = 0;
					...
					sub4("WAITING", 5, 5, 2.0);
					ActionStartConversation(GetFirstPC(), "", 0, 0, 0, "", "", "", "", "", "");
				}
				else {
					sub3(OBJECT_SELF);
					SetGlobalNumber("Tar_BullMerc", 2);
					sub5(GetObjectByTag("tar02_wpnpcext", 0), 1);
				}
			}
			break;
		case 2001:
			sub4("TALKING", 5, 5, 5.0);
			ActionStartConversation(GetFirstPC(), "", 0, 0, 0, "", "", "", "", "", "");
			break;
	}
}
