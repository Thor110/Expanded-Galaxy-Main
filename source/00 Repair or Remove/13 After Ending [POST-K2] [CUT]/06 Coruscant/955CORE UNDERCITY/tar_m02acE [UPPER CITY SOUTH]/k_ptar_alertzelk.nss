...
void main() {
	...
	object oZelka021 = GetObjectByTag("Zelka021", 0);
	sub1("FAILED TO OPEN", 5, 5, 5.0);
	if ((!sub2(OBJECT_SELF))) {
		if ((!sub3(OBJECT_INVALID))) {
			sub4(OBJECT_SELF, 1);
		}
		if ((GetGlobalNumber("tar_zelkarm") < 2)) {
			SetGlobalNumber("Tar_ZelkaRm", 1);
			NoClicksFor(1.2);
			DelayCommand(1.0, sub5("Zelka021", "", OBJECT_INVALID));
		}
		else {
			if ((GetGlobalNumber("tar_zelkarm") < 2)) {
		...
		if ((GetTag(object1) == "Zelka021")) {
			AssignCommand(object1, ActionResumeConversation());
