...
void main() {
	AddJournalQuestEntry("tar_vulkarbase", 1, 0);
	if (GetGlobalBoolean("Tar_SwoopRace")) {
		sub1(GetObjectByTag("tar10_vulkcoward", 0));
		sub1(GetObjectByTag("tar10_vulkcoward2", 0));
		sub1(GetObjectByTag("tar10_waitress", 0));
	}
	sub3(OBJECT_SELF);
	SetGlobalBoolean("TAR_VULKARBASE_OPEN", 1);
}
