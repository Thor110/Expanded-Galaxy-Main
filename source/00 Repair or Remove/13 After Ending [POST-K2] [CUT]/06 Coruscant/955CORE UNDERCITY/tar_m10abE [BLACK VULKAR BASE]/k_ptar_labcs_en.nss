...
// Prototypes
void sub4();
void sub3(string stringParam1, string stringParam2);
...
void sub3(string stringParam1, string stringParam2) {
	...
	if ((GetIsObjectValid(object1) == 1)) {
		if ((oPC == GetPartyMemberByIndex(0))) {
			...
			AssignCommand(object1, ClearAllActions());
			CancelCombat(oPC);
			AssignCommand(oPC, DelayCommand(1.0, JumpToObject(object1, 1)));
			AssignCommand(oNPC, DelayCommand(1.5, JumpToObject(oPC, 1)));
			AssignCommand(object8, DelayCommand(1.5, JumpToObject(oPC, 1)));
			AssignCommand(object1, DelayCommand(1.2, ActionStartConversation(oPC, stringParam2, 0, 0, 1)));
		}
	}
}
