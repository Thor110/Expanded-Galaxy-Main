...
void main() {
	...
	if (((GetIsPC(oEntering) && (!sub1(OBJECT_SELF))) && GetGlobalBoolean("tar_upelev"))) {
		sub2(OBJECT_SELF, 1);
		sub3("TRIGGER FIRED", 5, 5, 5.0);
		if (GetIsObjectValid(GetObjectByTag("Brutalized021", 0))) {
			sub3("ITHORIAN GOOD", 5, 6, 5.0);
		}
		SignalEvent(GetObjectByTag("Brutalized021", 0), EventUserDefined(2000));
		sub4(GetTag(OBJECT_SELF), "tar02_ithbrutal", OBJECT_INVALID);
	}
}
