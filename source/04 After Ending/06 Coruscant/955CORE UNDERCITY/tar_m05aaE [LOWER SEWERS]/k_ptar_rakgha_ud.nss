...
void sub5() {
	vector struct4;
	vector vPosition = GetPosition(OBJECT_SELF);
	object oNearestTar05_wploot = GetNearestObjectByTag("tar05_wploot", OBJECT_SELF, 1);
	int nRandom = Random(100);
	float float7 = (Random(6) + 7.0);
	...
	if (((GetIsObjectValid(oNearestTar04_gamraidleader) && (fDistance > 10.0)) && (fDistance < 30.0))) {
		...
		if ((nRandom < 0)) {
			...
			if (((nRandom < 70) && (GetDistanceBetween(OBJECT_SELF, oNearestTar05_wploot) < 10.0))) {
				SetFacingPoint(GetPosition(oNearestTar05_wploot));
				ActionPlayAnimation(107, 1.0, 0.0);
			}
			else {
				ActionWait(3.0);
									...
									if ((int1 == 1009)) {
										...
										if ((int1 == 1010)) {
											...
											if ((int1 == intGLOB_138)) {
												...
												ActionDoCommand(sub4(intGLOB_148));
												SetCommandable(0, OBJECT_SELF);
											}
										}
									}
