...
void sub4() {
	...
	object oAreaObject;
	oTar10_kandona001 = CreateObject(1, "tar10_kandona001", GetLocation(GetObjectByTag("tar10_wpkandon", 0)), 0);
	if (((!GetIsObjectValid(oTar10_kandona001)) || (!GetIsObjectValid(GetObjectByTag("tar03_wpkandon", 0))))) {
		sub1("KANDON BAD", 5, 6, 5.0);
	}
	else {
		sub1("KANDON GOOD", 5, 6, 5.0);
	}
	oAreaObject = GetFirstObjectInArea(OBJECT_INVALID, 1);
	while (GetIsObjectValid(oAreaObject)) {
		if (((GetObjectType(oAreaObject) == 1) && (GetStandardFaction(oAreaObject) == 1))) {
			DestroyObject(oAreaObject, 0.0, 1, 0.0);
		}
		oAreaObject = GetNextObjectInArea(OBJECT_INVALID, 1);
	}
	sub5(GetTag(oTar10_kandona001), "", OBJECT_INVALID);
}
...
void main() {
	...
	sub1("SPAWNING KANDOR", 5, 5, 5.0);
	if ((((GetIsPC(oEntering) && (!sub2(OBJECT_SELF))) && GetGlobalBoolean("Tar_GadonDead")) && (!GetGlobalBoolean("Tar_SwoopRace")))) {
		sub3(OBJECT_SELF, 1);
